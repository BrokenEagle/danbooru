class PostLock < ApplicationRecord
  ...
  ALL_LOCKS = ALL_TYPES.map { |type| "#{type}_lock" }
  ALL_CHANGES = ALL_TYPES.map { |type| "#{type}_lock_changed" }
  ...
  def self.search(params)
    ...
    ALL_LOCKS.each do |type|
      if params[type].to_s.truthy?
        q = q.bit_flags_match(type, true)
      elsif params[type].to_s.falsy?
        q = q.bit_flags_match(type, false)
      end
    end

    ALL_CHANGES.each do |type|
      if params[type].to_s.truthy?
        q = q.bit_changes_match(type, true)
      elsif params[type].to_s.falsy?
        q = q.bit_changes_match(type, false)
      end
    end
