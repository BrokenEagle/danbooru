class ForumTopic < ApplicationRecord
  ...
  def pending_bur_count
    bulk_update_requests.reject {|request| request.status != "pending"}.size
  end

  def approved_bur_count
    bulk_update_requests.reject {|request| request.status != "approved"}.size
  end

  def rejected_bur_count
    bulk_update_requests.reject {|request| request.status != "rejected"}.size
  end
