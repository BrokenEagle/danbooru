class DtextLink < ApplicationRecord
  ...
  def self.search(params)
    params[:has_linked_wiki] = params[:linked_wiki_exists] if params[:linked_wiki_exists].present?
    params[:has_linked_tag] = params[:linked_tag_exists] if params[:linked_tag_exists].present?
    q = super
  ...
  def self.attribute_restriction(name)
    if name == :linked_wiki || name == :linked_tag
