module Searchable
  ...
  def search_has_include(name, exists, model)
    ...
    primary_key, foreign_key = foreign_key, primary_key if [:has_many, :has_one].include?(association.macro) #:association.inverse_of.present?
    return all if primary_key.nil? || foreign_key.nil?

    self_table = arel_table
    model_table = model.arel_table
    model_exists = model.model_restriction(model_table).where(model_table[primary_key].eq(self_table[foreign_key])).exists
    if exists
