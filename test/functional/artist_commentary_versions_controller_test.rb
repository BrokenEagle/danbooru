...
class ArtistCommentaryVersionsControllerTest < ActionDispatch::IntegrationTest
  context "The artist commentary versions controller" do
    setup do
      ...
      as(@user) do
        @post = create(:post, id: 999, tag_string: "hakurei_reimu", uploader: @user)
        @commentary = create(:artist_commentary, post: @post)
      end
    ...
    context "index action" do
      setup do
        ...
        as(@builder) do
          @other_post = create(:post, uploader: @builder)
          @other_commentary = create(:artist_commentary, post: @other_post)
        end
